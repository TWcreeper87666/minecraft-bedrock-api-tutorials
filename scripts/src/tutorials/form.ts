...
async function form_actionForm(player: Player) {
...
async function form_modalForm(player: Player) {
  // 把能用的都列出來了，雖然如果你有自動補全的話根本不需要🤣
...
async function form_handle(player: Player) {
  const form = new ModalFormData()
    .title("§l§1表單處理")
  ...
  const [_num, toChat] = formValues as [string, boolean];
  const num = parseInt(_num);

  if (Number.isNaN(num)) return player.sendMessage("請輸入數字!");

  if (toChat) player.sendMessage(`你輸入了數字: ${num}`);
}
